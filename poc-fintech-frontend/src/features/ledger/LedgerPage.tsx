...
import { useLedgerByAccount, useLedgerByTransfer, useLedgerRecent } from '../../hooks/useApi';
            ...
            <tbody className="divide-y divide-gray-100">
              {query.data.map((entry) => (
                <tr key={entry.id} className="hover:bg-gray-50">
                  <td className="px-4 py-3 font-mono text-xs text-gray-500 break-all">{entry.id}</td>
                  <td className="px-4 py-3 font-mono text-xs text-red-600 break-all">{entry.debitAccountId}</td>
                  <td className="px-4 py-3 font-mono text-xs text-green-600 break-all">{entry.creditAccountId}</td>
                  <td className="px-4 py-3 text-right font-mono font-medium">
                    {formatCurrency(entry.amount, entry.currency)}
                  </td>
                  <td className="px-4 py-3 font-mono text-xs text-gray-500 break-all">{entry.transferId}</td>
